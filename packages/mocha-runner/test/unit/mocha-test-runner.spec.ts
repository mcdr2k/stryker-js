...
describe(MochaTestRunner.name, () => {
  ...
  describe('capabilities', async () => {
    const expectedCapabilities: TestRunnerCapabilities = { reloadEnvironment: false, simultaneousTesting: true, smartBail: true };
    ...
    it('should communicate smartBail=true', async () => {
  ...
  describe(MochaTestRunner.prototype.mutantRun.name, () => {
    ...
    it('should be able to report a killed mutant when a test fails', async () => {
      ...
      delete (result as any).startToMutantRunResult; //todo: fix this
      delete (result as any).endToMutantRunResult; //todo: fix this
